...
- **`setValueOrThrow()` does not cache the container itself.** The `$this !== $value` examination keeps `$this` out of `values`. The rule holds for every ID, not for `ContainerInterface::class` alone: a factory that gives the container is clone-relative under any ID you register it with. Thus a clone finds no cached entry and resolves the container anew. A value that the user registers is not `$this`, thus the cache keeps it and a clone reuses it as is.
- **The container cannot cache itself and let a subclass forbid cloning.** A cached self-reference needs a `__clone()` hook to correct the copy, and a declared `__clone()` is public, thus a subclass could no longer make it private. The uncached factory is the price of that ability. It costs about 230 ns for each `get(ContainerInterface::class)`: 0.4 us against 0.17 us for a cache hit. The `SelfResolution` benchmark measures this path. An autowired parameter pays the cost once for each dependent service, because `values` keeps the dependent.
