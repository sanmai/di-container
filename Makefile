...
MIN_MSI=100
MIN_COVERED_MSI=100
INFECTION_ARGS=--min-msi=$(MIN_MSI) --min-covered-msi=$(MIN_COVERED_MSI) --threads=$(JOBS) --coverage=build/logs --log-verbosity=default --show-mutations --no-interaction --with-uncovered
...
benchmark: prerequisites benchmarks/Fixtures/A/FixtureA1.php
	$(PHP) $(PHPBENCH) run --progress=verbose --report=aggregate | sed 's/+/|/g; 1d; $$d' | sed '3,$$ { /^|[-| ]*|$$/d; }' | tee -a $(GITHUB_OUTPUT)
